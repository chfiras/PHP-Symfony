...
class AdvertRepository extends EntityRepository
{
  ...
  public function getAdvertWithSiteWeb(array $SiteWebNames)
  {
    $qb = $this->createQueryBuilder('a');

    // On fait une jointure avec l'entité SiteWeb avec pour alias « c »
    $qb
      ->innerJoin('a.SiteWeb', 'c')
      ->addSelect('c')
    ;

    // Puis on filtre sur le nom des catégories à l'aide d'un IN
    $qb->where($qb->expr()->in('c.name', $SiteWebNames));
  ...
  public function getFiches()
  {
    ...
    public function getState($fid,$nid)
    {
        $query = $this->createQueryBuilder('a')
            ->leftJoin('a.NomDeDomaine','n')
            ->where('a.id='.$fid)
